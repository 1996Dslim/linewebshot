...
export default {
    ...
    setup(props, {emit}) {
        ...
        async function findSeries() {
            ...
            // 금형 > 생산관리 지표 (output indicator)
            const res = await StatisticApi.findMoldOutputIndicatorByMonthAndMold(
                yyyymm,
                props.id
            );
            if (res.data) {
                res.data.forEach((d) => {
                    ...
                    series.value[1].data.push(
                        isNaN(d.adjustedOutputCnt) ? 0 : d.adjustedOutputCnt
                    );
                    series.value[2].data.push(
                        d.dailyOutputRate ? d.dailyOutputRate : 0
                    );
                });
            }
        }
